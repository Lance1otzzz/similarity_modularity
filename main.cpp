...
/*
   ...
   (louvain_plus.hpp) algorithm 10: louvain algorithm 11: louvain_flm algorithm
   12: pp_with_both algorithm 13: pp_with_bipolar algorithm 14: pp_with_hybrid
...
int main(int argc, char **argv) {
  ...
  if (argc < 4) {
    ...
    cerr << "a number for algorithm choice, a string for dataset name, and a "
            "double for r"
         << endl;
  ...
  cout << "LoadGraph time: " << timeElapsed(startLoadingGraph, endLoadingGraph)
       << endl;
  cout << "-----------------------------------" << endl;
  ...
  int bipolar_k = 200;
  // if (g.n > 0) {
  //     const double avg_degree = (g.n > 0) ? (2.0 * static_cast<double>(g.m))
  //     / static_cast<double>(g.n) : 0.0; cout << "average degree = " <<
  //     avg_degree << endl; if (avg_degree > 0.0) {
  ...
  if (algorithm == 7 || algorithm == 12 || algorithm == 13 || algorithm == 14 ||
    ...
    preprocessing_time = build_bipolar_pruning_index(
        g, dataset_path, bipolar_k, 0,
        BipolarKMeansVariant::Yinyang); // 注意！！！！
  ...
  case 13: {
    ...
    // Main algorithm
    // pure_louvain_with_bipolar_pruning(g,r);//it's baseline louvain,not pure
    // louvain
  ...
  cout << " Main algorithm time: " << timeElapsed(startMainAlg, endMainAlg)
       << endl;
  ...
  if (algorithm==7||algorithm==12||algorithm==13||algorithm==14)
  {
          ...
          if (g_bipolar_pruning) {
                  cout<<"Total queries:
  "<<g_bipolar_pruning->get_total_queries()<<endl; cout<<"Successful prunings:
  "<<g_bipolar_pruning->get_pruning_count()<<endl; cout<<"Full calculations:
  "<<g_bipolar_pruning->get_full_calculations()<<endl; if
  (g_bipolar_pruning->get_total_queries() > 0) { double pruning_rate =
  (double)g_bipolar_pruning->get_pruning_count() /
  g_bipolar_pruning->get_total_queries() * 100.0; cout<<"Pruning rate:
  "<<pruning_rate<<"%"<<endl;
                  }
          }
  }
  */
  ...
  cout << "Total time cost: " << timeElapsed(startLoadingGraph, endMainAlg)
       << endl;
  return 0;
}
